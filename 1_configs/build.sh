...
do_build() {
    ...
    # src/ is grouped by concern (git/, gha/, apps/) to match every other repo
    # under cloud. dist/ stays FLAT: deploy paths and, more importantly,
    # core.hooksPath (1_configs/dist/hooks) are then identical everywhere, so
    # the same gitconfig works in every repo.
    for _f in gitconfig gitattributes gitignore gitmodules; do
        [ -f "$DIST_DIR/git/$_f" ] && mv "$DIST_DIR/git/$_f" "$DIST_DIR/$_f"
    done
    [ -d "$DIST_DIR/git/hooks" ]   && mv "$DIST_DIR/git/hooks"   "$DIST_DIR/hooks"
    [ -d "$DIST_DIR/gha/scripts" ] && mv "$DIST_DIR/gha/scripts" "$DIST_DIR/scripts"
    rmdir "$DIST_DIR/git" "$DIST_DIR/gha" 2>/dev/null || true
    unset _f
...
do_deploy() {
    ...
    if [ -f "$DIST_DIR/gitmodules" ]; then
        cp "$DIST_DIR/gitmodules" "$REPO_ROOT/.gitmodules"
...
# ── dotfiles ────────────────────────────────────────────────────────────────
# src/apps/<tool>/ → dist/apps/<tool>/ → <repo>/<target>/
...
case "${1:-all}" in
    ...
    test)     do_test ;;
    all|"")   do_build; do_deploy; do_dotfiles; do_test ;;
    *)        echo "Usage: $0 [build|deploy|dotfiles|test|all]" ;;
esac
