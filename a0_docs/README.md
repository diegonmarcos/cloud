...
```
cloud-master/unix         ->  ~/git/unix
cloud-master/tools        ->  ~/git/tools
cloud-master/2_vault/vault ->  ~/git/vault
```
...
`cloud-master/unix` and through `~/git/unix` are the same edits.

The links are **relative**, and that is what makes committing them work. Git
stores a symlink target verbatim, so an absolute `/home/diego/git/unix` would
